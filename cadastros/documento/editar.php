...
<?php
    ...
    if ( null==$id ) {
        header("Location: /sgpi/cadastros/documento");
    }
    ...
    if ( !empty($_POST)) {
        ...
        // update data
        if ($valid) {
			$sql = "UPDATE documento set descricao = '".$desc."' WHERE id = ".$id;
			if(mysql_query($sql)){
			...
			echo "<script>window.location.href='/sgpi/cadastros/documento';</script>";
        }
    } else {
